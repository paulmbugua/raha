...
function ModelCard({ model, index = 0 }: { model: UtamuModel; index?: number }) {
  ...
  const tier = String(model.listingTier || '').toLowerCase();
  const isVip = Boolean(model.elite || tier === 'vip');
  const tierLabel = isVip ? 'VIP' : ['bronze', 'silver', 'gold'].includes(tier) ? tier : '';
  ...
  const plans = [
    {
      ...
      highlight: 'From Ksh 600 weekly / Ksh 2,000 monthly',
    },
    {
      title: 'Register as Agency',
      price: 'From Ksh 300 weekly / Ksh 1,000 monthly',
      cta: 'Register here',
...
function MonetizationScreen() {
  ...
  const [method, setMethod] = useState<'mpesa' | 'paystack'>('paystack');
  ...
  useEffect(() => {
    ...
    if (next?.token) utamuApi.getMonetization(next.token).then((data: any) => {
      ...
      setAiAutoReply(data.assistant?.auto_reply_enabled !== false);
  ...
  async function waitForMonetizationPayment(reference: string) {
    ...
    setStatus('Still waiting for payment confirmation. This page will update automatically when you open Monetization again.');
    return false;
  }
  async function checkout(productId: string) {
    ...
    try {
      ...
      setStatus((result.instructions || 'Payment started.') + ' Waiting for payment confirmation...');
  ...
  async function saveAssistant(enabled = true) {
    if (!session?.token) return;
    try {
      const assistant = await utamuApi.configureAiAssistant({ enabled, tone: aiTone, instructions: aiInstructions, autoReplyEnabled: aiAutoReply }, session.token);
      setOverview((current: any) => ({ ...current, assistant }));
      setStatus(enabled ? 'AI assistant settings saved. Automatic replies now use your selected tone and notes.' : 'AI assistant paused.');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'AI assistant settings could not be saved.');
    }
  }
  ...
  const isMemberAccount = session.user?.accountType === 'member';
  const visibleProducts = (overview.products || []).filter((product: any) => isMemberAccount ? ['wallet', 'client_portal'].includes(product.category) : product.category !== 'wallet');
  const grouped = visibleProducts.reduce((acc: any, product: any) => ({ ...acc, [product.category]: [...(acc[product.category] || []), product] }), {});
  ...
  const productBillingLabel = (product: any) => {
    const tokens = product.tokenAmount || product.token_amount;
    if (tokens) return tokens + ' tokens';
    const days = Number(product.durationDays ?? product.duration_days ?? 30);
    if (days <= 7) return 'Weekly plan';
    if (days >= 28 && days <= 31) return 'Monthly plan';
    return days + ' days';
  };
  const listingTierForProduct = (product: any) => String(product.id || '').match(/^tier-(bronze|silver|gold|vip)(?:-(weekly|monthly))?$/)?.[1] || '';
  const listingExpiresAt = overview.model?.listing_tier_expires_at ? new Date(overview.model.listing_tier_expires_at) : null;
  const listingActive = Boolean(overview.model?.listing_tier && overview.model.listing_tier !== 'free' && (!listingExpiresAt || listingExpiresAt.getTime() > Date.now()));
  const assistantExpiresAt = overview.assistant?.expires_at ? new Date(overview.assistant.expires_at) : null;
  const assistantEntitlementActive = Boolean(overview.assistant?.activeEntitlement || (overview.assistant?.plan === 'assistant' && (!assistantExpiresAt || assistantExpiresAt.getTime() > Date.now())));
  const assistantEnabled = Boolean(overview.assistant?.enabled && assistantEntitlementActive);
  const productActive = (product: any) => {
    if (product.category === 'verification') return Boolean(overview.model?.trusted_badge || overview.model?.verified);
    if (product.category === 'listing') return listingActive && overview.model?.listing_tier === listingTierForProduct(product);
    if (product.category === 'ai') return assistantEntitlementActive;
    if (product.category === 'client_portal') return Boolean(overview.clientPortal);
    return false;
  };
  return <section className="grid gap-4 bg-[#fff7fb] px-4 py-6 lg:grid-cols-[minmax(0,1fr)_260px]"><div className="space-y-5"><div className="bg-gradient-to-r from-[#170421] via-[#2b0a3d] to-[#063b2c] p-5 text-white"><StatusBadge tone="gold">Revenue center</StatusBadge><h1 className="mt-3 text-3xl font-bold">Monetization</h1><p className="mt-2 max-w-3xl text-sm text-white/80">Sell trust badges, featured placement, tokenized messages, tips, AI assistant access, vetted client subscriptions, and booking leads from one account area.</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded border border-white/15 bg-white/10 p-3"><p className="text-xs uppercase text-white/60">Wallet</p><strong className="text-2xl">{overview.wallet?.balance_tokens || 0} tokens</strong></div><div className="rounded border border-white/15 bg-white/10 p-3"><p className="text-xs uppercase text-white/60">Listing tier</p><strong className="text-2xl capitalize">{overview.model?.listing_tier || 'free'}</strong></div><div className="rounded border border-white/15 bg-white/10 p-3"><p className="text-xs uppercase text-white/60">Trusted badge</p><strong className="text-2xl">{overview.model?.trusted_badge ? 'Active' : 'Not active'}</strong></div></div></div><div className="bg-white p-4 shadow-sm"><div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><select value={method} onChange={(event) => setMethod(event.target.value as 'mpesa' | 'paystack')} className={selectClass}><option value="paystack">Paystack</option><option value="mpesa">M-Pesa</option></select><input value={method === 'mpesa' ? phone : email} onChange={(event) => method === 'mpesa' ? setPhone(event.target.value) : setEmail(event.target.value)} className={fieldClass} placeholder={method === 'mpesa' ? '2547...' : 'email@example.com'} /><button onClick={() => checkout('tier-vip')} className="rounded-full bg-[#f0b323] px-5 py-2 text-center text-sm font-bold text-[#211000]">VIP monthly - Ksh 2,000</button></div>{status && <p className="mt-3 rounded bg-[#e6ffe9] p-3 text-sm text-[#147a33]">{status}</p>}</div>{Object.entries(grouped).map(([category, products]: any) => <section key={category} className="bg-white p-5 shadow-sm"><h2 className="border-l-4 border-[#e60073] pl-3 text-lg font-bold text-[#e60073]">{categoryTitles[category] || category}</h2><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{products.map((product: any) => <article key={product.id} className="rounded border border-[#ffd1e8] bg-[#fff8fb] p-4"><p className="text-xs font-bold uppercase text-[#006b3f]">{product.category}</p><h3 className="mt-2 text-xl font-bold text-[#2b0a3d]">{product.name}</h3><p className="mt-2 min-h-12 text-sm leading-6 text-[#7b6e78]">{product.description}</p><div className="mt-3 flex items-center justify-between"><strong className="text-lg text-[#111]">{kes(product.amountKes || product.amount_kes || 0)}</strong><span className={(product.tokenAmount || product.token_amount) ? "rounded-full bg-[#006b3f] px-2 py-1 text-xs font-bold text-white" : "rounded-full bg-[#f0b323] px-2 py-1 text-xs font-bold text-[#211000]"}>{productBillingLabel(product)}</span></div><button disabled={busyProduct === product.id || productActive(product)} onClick={() => checkout(product.id)} className={'mt-4 w-full rounded-full px-4 py-2 text-sm font-bold text-white disabled:opacity-80 ' + (productActive(product) ? 'bg-[#006b3f]' : 'bg-[#e60073]')}>{productActive(product) ? 'Active' : busyProduct === product.id ? 'Starting...' : product.category === 'wallet' ? 'Buy tokens' : 'Activate'}</button></article>)}</div></section>)}{!isMemberAccount && <section className="bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="border-l-4 border-[#006b3f] pl-3 text-lg font-bold text-[#006b3f]">AI assistant settings</h2><p className="mt-2 text-sm text-[#7b6e78]">Automatic replies activate after the AI assistant weekly or monthly add-on is paid.</p></div><span className={'rounded-full px-3 py-1 text-xs font-bold ' + (assistantEnabled ? 'bg-[#006b3f] text-white' : assistantEntitlementActive ? 'bg-[#fff0d0] text-[#7a4d00]' : 'bg-[#ffe1ef] text-[#9b0048]')}>{assistantEnabled ? 'Live' : assistantEntitlementActive ? 'Paid, paused' : 'Add-on inactive'}</span></div>{assistantExpiresAt && <p className="mt-3 text-xs text-[#7b6e78]">Paid access expires {assistantExpiresAt.toLocaleDateString('en-KE', { day: '2-digit', month: 'short', year: 'numeric' })}.</p>}<div className="mt-4 grid gap-3 sm:grid-cols-[220px_1fr]"><select value={aiTone} onChange={(event) => setAiTone(event.target.value)} className={selectClass}><option>polite</option><option>warm</option><option>direct</option><option>premium</option></select><input value={aiInstructions} onChange={(event) => setAiInstructions(event.target.value)} className={fieldClass} placeholder="Screening rules, availability notes, preferred response style" /></div><label className="mt-3 flex items-center gap-2 text-sm font-bold text-[#003b5c]"><input type="checkbox" checked={aiAutoReply} onChange={(event) => setAiAutoReply(event.target.checked)} />Auto-reply to new enquiries</label><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => assistantEntitlementActive ? saveAssistant(true) : checkout('ai-assistant-monthly')} className="rounded-full bg-[#006b3f] px-5 py-2 text-sm font-bold text-white">{assistantEntitlementActive ? 'Save and enable' : 'Activate AI add-on'}</button><button onClick={() => saveAssistant(false)} className="rounded-full border border-[#006b3f] px-5 py-2 text-sm font-bold text-[#006b3f]">Pause</button></div></section>}{!isMemberAccount && <section className="bg-white p-5 shadow-sm"><h2 className="border-l-4 border-[#e60073] pl-3 text-lg font-bold text-[#e60073]">Booking leads</h2><div className="mt-4 grid gap-3">{(overview.leads || []).length === 0 && <p className="text-sm text-[#7b6e78]">No paid leads yet.</p>}{(overview.leads || []).map((lead: any) => <article key={lead.id} className="rounded border border-[#ffd1e8] p-3 text-sm"><strong>{lead.model_name || 'Profile lead'}</strong><p className="mt-1 text-[#7b6e78]">{lead.message}</p><span className="text-xs text-[#006b3f]">Lead fee: {kes(Number(lead.lead_fee_kes || 0))}</span></article>)}</div></section>}</div><AccountSidebar active="monetization" /></section>;
}
...
function CheckoutScreen() {
  ...
  async function waitForPaymentStatus(reference: string) {
    ...
    setStatus('Still waiting for payment confirmation. You can safely keep this page open.');
  }
  async function pay() {
    setStatus('');
    setPaying(true);
    try {
      const payload = { amount: 2000, modelSlug: 'amina-w', purpose: 'vip_visibility', description: 'Secret Nairobi VIP visibility - 1 month' };
      if (method === 'mpesa') {
...
function ReviewScreen() {
  ...
  const reviewableModels = useMemo(() => sortDirectoryItems(directory.models.filter((model) => isIndependentModel(model) || isAgencyModel(model))), [directory.models]);
  const isNormalReviewer = session?.user?.accountType === 'member' && session?.user?.role === 'client';
  ...
  async function handleReviewSubmit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setReviewStatus('');
    if (!session?.token || !isNormalReviewer) {
      setReviewStatus('Only registered normal users can submit reviews.');
      return;
    }
    const form = new FormData(event.currentTarget);
    try {
      const item = await utamuApi.submitReview({ modelSlug: form.get('modelSlug'), rating: Number(form.get('rating') || 5), body: form.get('body') }, session.token);
      setRemoteReviews((current) => [item, ...current]);
      event.currentTarget.reset();
      setReviewStatus('Review submitted successfully.');
    } catch (error: any) {
      setReviewStatus(error?.message || 'Review could not be submitted.');
    }
  }

  return <div className="grid gap-0 bg-[#fff0f6] lg:grid-cols-[1fr_220px]"><section className="px-4 py-5 text-[#003b5c] md:px-5"><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-normal text-[#3b164b]">Reviews</h1><p className="mt-2 text-sm text-[#7b6e78]">Reviewed profiles and agencies are ranked by Trusted badges, VIP, New, then Independent signals.</p></div><div className="flex flex-wrap gap-2"><a href="#profile-reviews" className="rounded-[3px] bg-[#e60073] px-4 py-2 text-sm font-bold text-white">Profile reviews</a><a href="#agency-reviews" className="rounded-[3px] border border-[#e60073] px-4 py-2 text-sm font-bold text-[#e60073]">Agency reviews</a></div></div><section id="profile-reviews" className="space-y-4"><h2 className="border-l-4 border-[#ff1d9b] pl-3 text-lg font-bold uppercase text-[#ff1d9b]">Reviewed profiles</h2>{profileReviews.length === 0 && <p className="bg-white p-5 text-sm text-[#7b6e78]">No reviewed escort profiles have been submitted yet.</p>}{profileReviews.map((review: any) => <ReviewArticle key={review.id} review={review} />)}</section><section id="agency-reviews" className="mt-10 space-y-4"><h2 className="border-l-4 border-[#006b3f] pl-3 text-lg font-bold uppercase text-[#006b3f]">Reviewed agencies</h2>{agencyReviews.length === 0 && reviewedAgencyModels.length === 0 && <p className="bg-white p-5 text-sm text-[#7b6e78]">No agency reviews have been submitted yet.</p>}{agencyReviews.map((review: any) => <ReviewArticle key={review.id} review={review} />)}{reviewedAgencyModels.map((agency) => <article key={agency.id} className="rounded-[4px] border border-[#d8f0e4] bg-white p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-xl font-bold text-[#3b164b]">{agency.name}</h3><p className="mt-1 text-sm text-[#7b6e78]">{agency.city} - awaiting first review</p></div><a href={'/escort/' + agency.slug} className="rounded-full bg-[#006b3f] px-4 py-2 text-xs font-bold text-white">View agency</a></div></article>)}</section><form id="submit-review" onSubmit={handleReviewSubmit} className="mt-10 rounded-[3px] border border-[#ffd1e8] bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="border-l-4 border-[#ff1d9b] pl-3 font-bold uppercase text-[#ff1d9b]">Submit a review</h2><p className="mt-2 text-sm text-[#7b6e78]">Only normal user accounts can review independent escorts and agencies.</p></div>{session?.token && isNormalReviewer && <span className="rounded-full bg-[#ecfff5] px-3 py-1 text-xs font-bold text-[#006b3f]">Reviewing as {session.user?.fullName || session.user?.username || 'member'}</span>}</div>{!session?.token && <div className="mt-4 rounded-[3px] bg-[#fff0f6] p-4 text-sm text-[#3b164b]">Create or login to a normal user account to submit a review. <a className="font-bold text-[#e60073] underline" href="/register/member">Register as Normal User</a> or <a className="font-bold text-[#e60073] underline" href="/login">login</a>.</div>}{session?.token && !isNormalReviewer && <div className="mt-4 rounded-[3px] bg-[#fff0f6] p-4 text-sm text-[#3b164b]">Escort and agency accounts can receive reviews, but cannot submit them. Please use a normal user account for reviews.</div>}<div className="mt-4 grid gap-3 sm:grid-cols-2"><select name="modelSlug" className={selectClass} disabled={!isNormalReviewer || !reviewableModels.length}>{reviewableModels.length ? reviewableModels.map((model) => <option key={model.id} value={model.slug}>{model.name} - {isAgencyModel(model) ? 'Agency' : 'Independent escort'}</option>) : <option>No real profiles yet</option>}</select><select name="rating" className={selectClass} disabled={!isNormalReviewer}>{[5, 4, 3, 2, 1].map((rating) => <option key={rating} value={rating}>{rating} stars</option>)}</select></div><textarea name="body" required disabled={!isNormalReviewer} placeholder="Write a concise review after using the service" className="mt-3 min-h-28 w-full border border-[#ff55c7] p-3 text-sm outline-none disabled:bg-[#f8f1f5] disabled:text-[#9b8090]" />{reviewStatus && <p className="mt-3 rounded-[3px] bg-[#fff0f6] px-3 py-2 text-sm font-semibold text-[#3b164b]">{reviewStatus}</p>}<button disabled={!isNormalReviewer || !reviewableModels.length} className="mt-3 rounded-full bg-[#ff4eb8] px-5 py-2 text-sm font-bold text-white disabled:opacity-50">Submit review</button></form></section><RegistrationQuickSearch /></div>;
}
